...
export default {
  ...
  mounted() {
    if(localStorage.getItem('historyList6')) {
      this.historyList = JSON.parse(localStorage.getItem('historyList6'))
    }
  ...
  methods: {
    deletehistoryList(index) {
      ...
      localStorage.setItem('historyList6', JSON.stringify(this.historyList))
    },
    CloseHistory(val) {
      this.name = val
